...
其中${B\rho}$为磁刚度：
\begin{equation}
    {B\rho}=\frac{mv}{q} \approx \frac{mv_z}{q}
\end{equation}
...
如小节\ref{section:background}中介绍，空间电荷效应的来源为束团内的粒子之间的库伦相互作用。
...
解析的计算需要联立求解牛顿方程和麦克斯韦方程组（式\ref{eq:Newton}和\ref{eq:Maxwell}），其精确解必须使用数值的方法来得到。
...
因为PIC算法能够有效地降低运算量，所以绝大多数加速器束流模拟程序使用PIC算法来求解空间电荷力。
关于PIC算法的详细介绍将会在第\ref{section:PIC_algorithm}节中展开。

PIC算法需要将粒子权重到网格上，这样不可避免的会带来网格热噪声。另外PIC算法是否可以保障辛条件目前仍然有较大的争议。
...
当束流能量大于~10MeV~时，由于其束流能量较高，单腔能量增益对束流运行的状态相比低能段要弱很多。
在这部分，C-ADS~主要通过串联的方式进行备份。即主加速器只有一条线路，但是其工作参数低于其可以达到的正常的运行参数，以留出调整空间。
